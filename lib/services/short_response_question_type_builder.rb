module Services
  class ShortResponseQuestionTypeBuilder
    ...
    def find_or_create_skill_and_rubric_element_for_point_value(value)
      elements_for_point_value(value).map do |element_text|
        skill = find_or_create_associated_skill(element_text, value)
        RubricElement.find_or_create_by!(required_for_point_level: value, rubric: short_response_rubric, text: element_text, skill: skill)
      end
    end
